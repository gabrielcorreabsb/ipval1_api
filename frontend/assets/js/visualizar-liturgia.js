...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Funções de Armazenamento com Expiração ---
    const ANNOTATION_EXPIRATION_DAYS = 30;
    ...
    // --- Event Listeners ---
    if (closeDetailModalBtn) closeDetailModalBtn.addEventListener('click', hideLiturgyDetailModal);
    if (liturgyDetailModal) liturgyDetailModal.addEventListener('click', (e) => { if (e.target === liturgyDetailModal) hideLiturgyDetailModal(); });
    if (btnCompileShareWhatsapp) btnCompileShareWhatsapp.addEventListener('click', compileAndShareViaWhatsapp);
    if (btnDownloadLiturgyPDF) btnDownloadLiturgyPDF.addEventListener('click', generateAndDownloadLiturgyPDF);
    if (selectMonthEl) selectMonthEl.addEventListener('change', carregarLiturgiasDoMesSelecionado);
    if (selectYearEl) selectYearEl.addEventListener('change', carregarLiturgiasDoMesSelecionado);
    ...
    async function carregarLiturgiasDoMesSelecionado() {
        if (!selectMonthEl || !selectYearEl || !liturgiesForMonthContainer) return;
        ...
        try {
            if (todasAsLiturgiasGlobais.length === 0) {
                todasAsLiturgiasGlobais = await getLiturgias() || [];
            }
            ...
            if (liturgiasDoMes.length === 0) {
                ...
                liturgiasDoMes.forEach(liturgia => {
                    ...
                    li.innerHTML = `
                        <div class="liturgy-item-info"><h3>${liturgia.titulo}</h3><p class="date">Data: ${new Date(liturgia.data + "T00:00:00Z").toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</p></div>
                        <div class="liturgy-item-actions"><button class="btn-view-liturgy"><i class="fas fa-eye"></i> Ver Detalhes</button></div>`;
    ...
    async function displayLiturgyInModal(liturgiaId) {
        if(!liturgyDetailModal || !modalTitleEl || !modalItemsContainer || !modalBibleVersionSelect) return;

        const currentAnnotationsKey = `annotations_liturgy_${liturgiaId}`;
        const currentMarkedVersesKey = `marked_verses_liturgy_${liturgiaId}`;
        userAnnotations = getDataWithExpiry(currentAnnotationsKey) || {};
        markedVerses = getDataWithExpiry(currentMarkedVersesKey) || {};
            ...
            if (liturgiaData.itens && liturgiaData.itens.length > 0) {
                liturgiaData.itens.sort((a,b) => a.ordem - b.ordem).forEach(item => {
                    ...
                    if (item.tipo === 'TITULO_SECAO') {
                        itemDiv.innerHTML = `<h3>${item.conteudoTextual || 'Título'}</h3>`;
                        modalItemsContainer.appendChild(itemDiv);
                        return;
                    }

                    const mainContentDiv = document.createElement('div');
                    mainContentDiv.classList.add('item-main-content');
                    ...
                    switch (item.tipo) {
                        ...
                        case 'HINO':
                            ...
                            contentHtml = `<div class="hino-content"><p class="hino-title">${hParts[0] ? hParts[0].trim() : 'Hino'}</p>${hParts[1] ? `<p class="hino-number">${hParts[1].trim()}</p>` : ''}</div>`;
                            mainContentDiv.innerHTML = contentHtml;
                            break;
                        case 'REFERENCIA_BIBLICA':
                            ...
                            mainContentDiv.appendChild(refLnk);
                            ...
                            refLnk.addEventListener('click', async (e) => {
                                ...
                                if (isOpen && (accCont.innerHTML === '' || accCont.innerHTML.includes('loading-indicator'))) {
                                    ...
                                    await displayBibleVerseInAccordion(modalBibleVersionSelect.value, item.livroBiblia, item.capituloBiblia, item.versiculoInicioBiblia, item.versiculoFimBiblia, accCont, itemKey);
                                } else if (!isOpen) { accCont.innerHTML = ''; }
                            });
                            itemDiv.appendChild(accCont);
                            break;
                        default:
                            mainContentDiv.innerHTML = `<p><em>${item.conteudoTextual || 'Item não especificado'}</em></p>`;
                    }
                    ...
                    mainContentDiv.appendChild(annotationToggleBtn);
                    ...
                    annotationToggleBtn.onclick = () => {
                        annotationTextarea.classList.toggle('visible');
                        if (annotationTextarea.classList.contains('visible')) annotationTextarea.focus();
                    };
                    annotationTextarea.oninput = () => {
                        userAnnotations[itemKey] = annotationTextarea.value.trim();
                        saveDataWithExpiry(currentAnnotationsKey, userAnnotations, ANNOTATION_EXPIRATION_DAYS);
                    };
                    itemDiv.appendChild(annotationTextarea);
                    modalItemsContainer.appendChild(itemDiv);
                });
    ...
    async function displayBibleVerseInAccordion(versionSiglaApi, livroEnumNome, capitulo, vInicio, vFim, targetDiv, parentItemKey) {
        ...
        try {
            const versiculosArray = await fetchVerseRangeFromBibleDigital(versionSiglaApi, abrevLivroApi, capitulo, vInicio, vFim || vInicio);
            if (versiculosArray && versiculosArray.length > 0) {
                ...
                versiculosArray.forEach(v => {
                    ...
                    const savedAnnotation = markedVerses[verseKey]?.annotation || '';
                    versesHtml += `
                        <div class="verse-wrapper">
                            <p class="verse ${isMarked ? 'marked' : ''}" data-verse-key="${verseKey}" data-verse-text="${v.text}" data-verse-ref="${livroSelecionado.nomeCompleto} ${capitulo}:${v.number}">
                                <sup>${v.number}</sup> ${v.text}
                            </p>
                            <textarea class="verse-annotation-input" data-verse-key-for-annotation="${verseKey}" placeholder="Sua nota sobre ${livroSelecionado.nomeCompleto} ${capitulo}:${v.number}..." style="display:${isMarked ? 'block' : 'none'}">${savedAnnotation}</textarea>
                        </div>`;
                });
            ...
            } else { targetDiv.innerHTML = '<p style="padding:10px;">Texto bíblico não encontrado.</p>'; }
        } catch (error) {
    ...
    function toggleVerseMark(verseElement) {
        ...
        const currentMarkedVersesKey = `marked_verses_liturgy_${liturgiaAtualNoModal.id}`;
        saveDataWithExpiry(currentMarkedVersesKey, markedVerses, ANNOTATION_EXPIRATION_DAYS);
    }
    ...
    function compileAndShareViaWhatsapp() {
        ...
        const sortedMarkedVerseKeys = Object.keys(markedVerses).sort((a,b)=>{const pA=a.split('-');const pB=b.split('-');return (parseInt(pA[2])-parseInt(pB[2]))||(parseInt(pA[3])-parseInt(pB[3]))||(parseInt(pA[4])-parseInt(pB[4]));});
    ...
    async function generateAndDownloadLiturgyPDF() {
        if (!jsPDF) { mostrarMensagem("PDF indisponível.", "error"); console.error("jsPDF não carregado."); return; }
        if (!liturgiaAtualNoModal) { mostrarMensagem("Nenhuma liturgia ativa.", "info"); return; }
        Swal.fire({ title: 'Gerando PDF...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        try {
            const doc = new jsPDF({orientation: 'p', unit: 'mm', format: 'a4'});
            let yPosition = 15; const pageWidth = doc.internal.pageSize.getWidth();
            const margin = 15; const usableWidth = pageWidth - 2 * margin;
            function addProcessedText(text, x, y, options = {}) {
                const fontSize = options.fontSize || 10; const fontStyle = options.fontStyle || "normal";
                const color = options.color || [0,0,0]; const align = options.align || 'left';
                const lineSpacingFactor = options.lineSpacingFactor || 1.2; const isHtml = options.isHtml || false;
                doc.setFontSize(fontSize); doc.setFont("helvetica", fontStyle); doc.setTextColor(color[0],color[1],color[2]);
                let lines; const textToSplit = text || "";
                if (isHtml) {
                    const textParts = textToSplit.split('<br>'); lines = [];
                    textParts.forEach(part => { lines = lines.concat(doc.splitTextToSize(part, usableWidth - (align === 'center' ? 0 : (x - margin)))); });
                } else { lines = doc.splitTextToSize(textToSplit, usableWidth - (align === 'center' ? 0 : (x - margin))); }
                const textBlockHeight = lines.length * (fontSize * 0.352778) * lineSpacingFactor;
                if (y + textBlockHeight > doc.internal.pageSize.getHeight() - margin - 10) { doc.addPage(); y = margin; }
                doc.text(lines, x, y, { align: align });
                return y + textBlockHeight + (options.customSpacingAfter || 0);
            }
            const logoOriginalWidth = 200; const logoOriginalHeight = 139; const desiredLogoWidthMM = 50;
            const logoHeightMM = (logoOriginalHeight / logoOriginalWidth) * desiredLogoWidthMM;
            const logoX = (pageWidth - desiredLogoWidthMM) / 2; const logoY = yPosition;
            try {
                if (LOGO_IPV1_BASE64 && LOGO_IPV1_BASE64.startsWith('data:image')) {
                    ...
                    yPosition += logoHeightMM + 5;
                }
            } catch (e) { console.error("Erro ao adicionar logo ao PDF:", e); }
            yPosition = addProcessedText("Igreja Presbiteriana de Valparaíso 1", pageWidth / 2, yPosition, { fontSize: 14, fontStyle: 'bold', align: 'center', color: [8, 74, 39], customSpacingAfter: 8 });
            yPosition = addProcessedText(liturgiaAtualNoModal.titulo, pageWidth / 2, yPosition, { fontSize: 16, fontStyle: 'bold', align: 'center', color: [11, 102, 54], customSpacingAfter: 3 });
            yPosition = addProcessedText(`Data: ${new Date(liturgiaAtualNoModal.data + "T00:00:00Z").toLocaleDateString('pt-BR', {weekday:'long',year:'numeric',month:'long',day:'numeric',timeZone:'UTC'})}`, pageWidth/2, yPosition, { fontSize:10, fontStyle:'italic', align:'center', color:[80,80,80], customSpacingAfter: 12 });
            liturgiaAtualNoModal.itens.sort((a,b)=>a.ordem-b.ordem).forEach(item=>{
                let pfx="",txt=item.conteudoTextual||"",fs=11,fSty="normal",clr=[0,0,0],sTitle=false,isHtml=false;
                switch(item.tipo){
                    case 'TITULO_SECAO': txt=(item.conteudoTextual||"Seção").toUpperCase();fs=13;fSty='bold';clr=[8,74,39];sTitle=true;break;
                    case 'HINO': pfx="Hino: ";fSty='italic';clr=[50,50,50];break;
                    case 'REFERENCIA_BIBLICA': pfx="Leitura: ";txt=item.referenciaDisplayFormatada||"";fSty='bolditalic';clr=[11,102,54];break;
                    case 'ORACAO_COMUNITARIA':pfx="Oração: ";fSty='italic';if(item.conteudoTextual)isHtml=true;break;
                    case 'ANUNCIO':pfx="Anúncio: ";if(item.conteudoTextual)isHtml=true;break;
                    case 'TEXTO_SIMPLES':if(item.conteudoTextual)isHtml=true;break;
                }
                let fTxt=(pfx+txt).trim();
                if(fTxt){
                    yPosition = addProcessedText(fTxt, margin, yPosition, {fontSize:fs,fontStyle:fSty,color:clr,isHtml:isHtml,customSpacingAfter:(userAnnotations[`item-${liturgiaAtualNoModal.id}-${item.ordem}`]?1:3)});
                    if(sTitle){doc.setLineWidth(0.2);doc.setDrawColor(220,220,220);doc.line(margin,yPosition-2,pageWidth-margin,yPosition-2);doc.setDrawColor(0);yPosition+=1;}
                }
                const iKey=`item-${liturgiaAtualNoModal.id}-${item.ordem}`;
                if(userAnnotations[iKey])yPosition=addProcessedText(`    Anotação: ${userAnnotations[iKey]}`,margin+5,yPosition,{fontSize:9,fontStyle:'italic',color:[100,100,100],customSpacingAfter:3});
                yPosition += (fTxt||userAnnotations[iKey])?4:0;
            });
            const sMK = Object.keys(markedVerses).sort((a,b)=>{const pA=a.split('-');const pB=b.split('-');return (parseInt(pA[2])-parseInt(pB[2]))||(parseInt(pA[3])-parseInt(pB[3]))||(parseInt(pA[4])-parseInt(pB[4]));});
            if(sMK.length>0){
                yPosition+=8; yPosition=addProcessedText("VERSÍCULOS MARCADOS",margin,yPosition,{fontSize:13,fontStyle:'bold',color:[11,102,54],customSpacingAfter:2});
                ...
                sMK.forEach(vK=>{
                    const vD=markedVerses[vK];
                    yPosition=addProcessedText(`${vD.reference}:`,margin,yPosition,{fontSize:10,fontStyle:'bolditalic',color:[8,74,39]});
                    yPosition=addProcessedText(`    “${vD.text}”`,margin+5,yPosition,{fontSize:10,fontStyle:'italic',color:[50,50,50],customSpacingAfter:(vD.annotation?1:3)});
                    if(vD.annotation)yPosition=addProcessedText(`        Nota: ${vD.annotation}`,margin+10,yPosition,{fontSize:9,fontStyle:'italic',color:[100,100,100],customSpacingAfter:3});
                    yPosition+=2;
                });
            }
            const finalY=doc.internal.pageSize.getHeight()-10;
            if(finalY>yPosition+10){doc.setFontSize(8);doc.setFont("helvetica","italic");doc.setTextColor(150,150,150);doc.text("Gerado por Sistema IPV1",pageWidth/2,finalY,{align:'center'});}
            const ts=new Date().toISOString().replace(/[:.]/g,'-').slice(0,19);
            doc.save(`liturgia_${liturgiaAtualNoModal.titulo.replace(/[^a-z0-9]/gi,'_').slice(0,25).toLowerCase()}_${ts}.pdf`);
            Swal.close(); mostrarMensagem("PDF gerado!", "success");
        }catch(err){console.error("Erro ao gerar PDF:",err);Swal.close();mostrarMensagem("Erro ao gerar PDF.", "error");}
    }
